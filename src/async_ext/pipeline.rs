...
// Retry-related methods (requires async-retry feature)
#[cfg(feature = "async-retry")]
mod retry_impl {
    use super::*;
    use core::time::Duration;

    use crate::async_ext::retry::{retry_with_policy, RetryPolicy};
    ...
    where
        ...
        E: TransientError,
    {
        /// Retries the operation if it fails with a transient error.
        ///
        /// This method uses the provided retry policy and sleep function to
        /// implement retry logic in a runtime-neutral way.
        ///
        /// # Arguments
        ///
        /// * `operation_factory` - A closure that creates the future to retry
        ...
        /// ).await;
        /// ```
        pub async fn retry_if_transient_with<F, Fut2, P, S, SFut>(
            operation_factory: F,
            policy: P,
            sleep_fn: S,
        ...
        where
            ...
            SFut: Future<Output = ()>,
        {
            retry_with_policy(operation_factory, policy, sleep_fn).await
        }
    }
}
