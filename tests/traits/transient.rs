...
fn test_retry_if_transient() {
    ...
    let transient_err: Result<i32, TestError> = Err(TestError {
        transient: true,
        retry_after: None,
    });
    assert!(transient_err.retry_if_transient().is_some());

    let permanent_err: Result<i32, TestError> = Err(TestError {
        transient: false,
        retry_after: None,
    });
